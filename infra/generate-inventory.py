...
def generate_inventory():
    command = "terraform output --json manager_vm_ips".split()
    ip_data = json.loads(run(command).stdout)
    manager_node = ip_data.pop()

    host_vars = {}

    host_vars[manager_node] = { "ip": [manager_node] }
    ...
    _all = { "children": ["managernode", "storagenode", "workers"] }

    _workers = { "hosts": workers }
    _storagenode = { "hosts": [storage_node] }
    _managernode = { "hosts" : [manager_node] }

    _jd = {}
    _jd["_meta"] = _meta
    _jd["all"] = _all
    _jd["workers"] = _workers
    _jd["managernode"] = _managernode
